...
function _errorValidation(isInitializing = false) {
  ...
  groupLog(_testEnv, 'Error validation data', logArray);

  if (typeof _walkMeID === 'undefined' || _walkMeID === '') {
    log(_testEnv, '[_errorValidation] Failed >> WalkMe ID variable not available', null, 'warn');
    return ERROR_CODES.WALK_ME_ID;
  }

  if (typeof _testEnv === 'undefined' || _testEnv === null) {
    log(_testEnv, '[_errorValidation] Failed >> testEnv variable not available', null, 'warn');
    return ERROR_CODES.ENVIRONMENT;
  }

  if (typeof window === 'undefined') {
    log(_testEnv, '[_errorValidation] Failed >> window is undefined', null, 'warn');
    return ERROR_CODES.WINDOW;
  }

  if (isInitializing === false && !window._walkMe) {
    log(_testEnv, '[_errorValidation] Failed >> _walkMe is not available.', null, 'warn');
    return ERROR_CODES.WALK_ME;
  }

  log(_testEnv, '[_errorValidation] Success >> Validations passed!');
 ...
 * @returns {object}
 */
export function initialize(walkMeID, testEnv, callback = null) {
  const logArray = [
    {
  ...
  groupLog(testEnv, 'WalkMe Initialization data', logArray);
